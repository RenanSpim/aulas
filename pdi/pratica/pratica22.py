...
def main():
    ...
    for direcao, cor_pixel in vizinhanca.items():
        ...
        if distancia_vizinho <= tolerancia:
            ...
            # Calcula a distância de TODOS os pixels da imagem em relação a este vizinho
            # axis=-1 garante que o cálculo seja feito sobre os canais R, G e B
            distancias_imagem = np.linalg.norm(imagem_int32 - cor_pixel_int, axis=-1)
            
            # Máscara: todos os pixels cuja distância seja menor ou igual à tolerância
            mask = distancias_imagem <= tolerancia
            
            # Pinta os pixels correspondentes de vermelho vivo [255, 0, 0]
